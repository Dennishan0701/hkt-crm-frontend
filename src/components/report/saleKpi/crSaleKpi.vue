                ...
                <table>
                  <tbody>
                    <tr v-for="(item,index) in data" :key='index' :title="item.userName+'：'+item.userType">
                      ...
                      <!--渠道等级K-->
                      <td>{{(item.k_sumRealityMonery || item.k_sumRealityMonery == 0) ? item.k_sumRealityMonery : '-'}}</td>
                      <td>{{(item.k_realityMonery || item.k_realityMonery == 0) ? item.k_realityMonery : '-'}}</td>
                      <td>{{(item.k_makeContractPeople || item.k_makeContractPeople == 0) ? item.k_makeContractPeople : '-'}}</td>
                      <td>{{(item.k_averageSingleBottom || item.k_averageSingleBottom == 0) ? item.k_averageSingleBottom : '-'}}</td>
                      <td>{{(item.k_leadsLists || item.k_leadsLists == 0) ? item.k_leadsLists : '-'}}</td>
                      <td>{{(item.k_arrangeCourseNumber || item.k_arrangeCourseNumber == 0) ? item.k_arrangeCourseNumber : '-'}}</td>
                      <td>{{(item.k_arrangeCourseRate || item.k_arrangeCourseRate == 0) ? item.k_arrangeCourseRate+'%' : '-'}}</td>
                      <td>{{(item.k_arrangeCourseChangeRate || item.k_arrangeCourseChangeRate == 0) ? item.k_arrangeCourseChangeRate+'%' : '-'}}</td>
                      <td>{{(item.k_totalChangeRate || item.k_totalChangeRate == 0) ? item.k_totalChangeRate+'%' : '-'}}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
