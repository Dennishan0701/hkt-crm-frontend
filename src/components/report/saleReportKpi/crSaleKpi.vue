  ...
  <div class="total-sale-kpi">
    <div class="searchForm">
      <Row>
        <i-col span=24 class="right">
          ...
          <i-button :size="$store.state.searchModuleSize" type="primary" icon="search" @click="search">查询</i-button>
    ...
    <div class="content" @scroll=handleScroll ref="viewBox">
      <table class="report-form">
        <thead>
          <tr>
            <th colspan="12" class="bg-total">CR_KPI</th>
          ...
          <tr class="bg-total">
            <!--总计-->
            <th class="bg-total name2">姓名</th>
            <th class="bg-total name2">团队</th>
            <th class="bg-total">职位（CR）</th>
        ...
        <tbody>
          <tr>
            <td colspan="58" style="padding: 0;border: 0;">
                ...
                <table>
                  <tbody>
                    <tr v-for="(item,index) in data" :key='index' :title="item.userName+'：'+item.userType">
                      ...
                      <!--渠道等级G-->
                      <td>{{(item.e_sumRealityMonery || item.g_sumRealityMonery == 0) ? item.g_sumRealityMonery : '-'}}</td>
                      <td>{{(item.e_realityMonery || item.g_realityMonery == 0) ? item.g_realityMonery : '-'}}</td>
                      <td>{{(item.e_makeContractPeople || item.g_makeContractPeople == 0) ? item.g_makeContractPeople : '-'}}</td>
                      <td>{{(item.e_averageSingleBottom || item.e_averageSingleBottom == 0) ? item.g_averageSingleBottom : '-'}}</td>
                      <td>{{(item.e_leadsLists || item.g_leadsLists == 0) ? item.g_leadsLists : '-'}}</td>
                      <td>{{(item.e_arrangeCourseNumber || item.g_arrangeCourseNumber == 0) ? item.g_arrangeCourseNumber : '-'}}</td>
                      <td>{{(item.e_arrangeCourseRate || item.g_arrangeCourseRate == 0) ? item.g_arrangeCourseRate+'%' : '-'}}</td>
                      <td>{{(item.e_arrangeCourseChangeRate || item.g_arrangeCourseChangeRate == 0) ? item.g_arrangeCourseChangeRate+'%' : '-'}}</td>
                      <td>{{(item.e_totalChangeRate || item.g_totalChangeRate == 0) ? item.g_totalChangeRate+'%' : '-'}}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
  ...
  export default{
    ...
    methods: {
      handleScroll(e) {
        ...
        var first = document.getElementsByClassName("td-data-list")[2].getElementsByTagName("tr");
        var name2 = document.getElementsByClassName("name2");
        for(let i = 0; i<name2.length;i++){
          let nameChild = name2[i]
//          console.log(first[i].firstElementChild);
      ...
      exportData() {
        ...
        if (date.length>0) {
          startTime = date[0];
          endTime = date[1];
        let apiUrl = REPORT().Sale.generateCrUserKpiExcel;
      ...
      loadDataGrid(startTime, endTime,type) {
        ...
        const _this = this;
        common.ajax({
          url: REPORT().Sale.crSaleKpi + param,
          method: 'get',
