<template>
  <div class="mc">
    <o-breadcrumb second="数据报表" third="推广leads报表"></o-breadcrumb>
...
<script type="text/ecmascript-6">
  ...
  export default{
    data() {
      return {
        data: [],
        tableColumnsChecked:['leadsName','leadsPhone','grade','subject','phoneLocation','signUpDate','jh','dy',
          'keyNum','saleName','isManagerCourse','isPay','payTime','leadPhone'],
        historyRemarkList:[],
        channeList:[],
        total: 0,
        pageIndex: 1,
...
//              return `<i-button type="warning" size="small" icon="social-twitch-outline" title="获取历史备注信息" @click="getStudentHistoryInfo('${row.leadsPhone}')"></i-button>`
      ...
      loadDataGrid(search, orderBy = '', sortBy = '') {
        ...
        const _this = this;
        common.ajax({
          url: REPORT().Leads.getNoRepeatRecommendLeadsKpi + param,
          method: 'get',
          success: function (res) {
