<template>
  <div class="mc">
    <o-breadcrumb second="数据报表" third="教学监课表"></o-breadcrumb>
    <div class="grid-widget">
      <Row>
        <i-col span=24 class="right">
          ...
          <i-select :size="$store.state.searchModuleSize" v-model="Search.feedbackStaus" clearable placeholder="课程反馈状态" style="width:120px;">
        ...
        <i-col span=24 class="right">
          <i-input :size="$store.state.searchModuleSize" v-model="Search.channelName" clearable placeholder="渠道名称" style="width:200px;margin-right:2px;"></i-input>
...
<script type="text/ecmascript-6">
  ...
  export default{
    data() {
      return {
        data: [],
        tableColumnsChecked:['grade','subject','courseType','courseLevel','length','courseDate','tcName','teacherPhone','natureOfWorkDesc','sale','team','channelName','feedbackDesc','ispayDesc'],
        total: 0,
        pageIndex: 1,
      ...
      getTableColumns () {
        const columns = {
          ...
          align:'center',
            minWidth: 120,
            render(h, params) {
          return h('span',params.row.courseDate+' '+params.row.startTime)
        }
        },
          tcName:{
          title: '老师姓名',
...
//            return hideCharAtNumber(row.teacherPhone) + ` <Icon type="ios-eye" @click="getFullPhone('${row.teacherUuid}')" class="eye" title="显示全部"></Icon>`;
      ...
      exportData() {
        ...
        apiUrl += '?courseStartTime='+ this.Search.courseDateStart + '&courseSEndTime=' + this.Search.courseDateEnd;
      ...
      loadDataGrid(search, orderBy = '', sortBy = '') {
        let param = `?feedbackStaus=${search.feedbackStaus}&courseDateStart=${search.courseDateStart}&courseDateEnd=${search.courseDateEnd}&subject=${search.subject}&grade=${search.grade}&type=${search.type}&level=${search.level}&tcName=${search.tcName}&sale=${search.sale}&team=${search.team}&channelName=${search.channelName}`;
