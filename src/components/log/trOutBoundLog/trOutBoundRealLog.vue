<template>
  <div class="businessLog">
    <o-breadcrumb second="日志监控" third="天润外呼实时记录"></o-breadcrumb>
    <div class="grid-widget">
      <Row>
        <i-col span=24 class="right">
          <i-input v-model="searchData.leadsParam" :size="$store.state.searchModuleSize" @on-keyup.enter="search(1)" placeholder="学生姓名/手机号码" style="width:140px;margin-right:3px;"></i-input>
...
//                      <i-button type="primary" size="small" @click="downloadVoiceFile('${row.uniqueId}','${row.recordFile}')">下载音频</i-button>`;
    ...
    methods: {
      ...
      loadDataGrid(search, orderBy = '', sortBy = '') {
        // let param = `?leadsParams=${search.leadsParam}&saleName=${search.userParam}&saleType=${search.userType}&team=${search.teamName}&startTime=${search.courseStartDate}&endTime=${search.courseEndDate}`;
        // param += `&pageIndex=${this.pageIndex}&pageNumber=${this.pageNumber}&orderBy=${orderBy}&sortBy=${sortBy}`;
        // const _this = this;
        // common.ajax({
        //   url: SYS_BUSINESS().DebugCourse.queryRealTimeOutBoundPage + param,
        //   method: 'get',
        ...
        this.$axios.get( SYS_BUSINESS().DebugCourse.queryRealTimeOutBoundPage,{
          params:{
      ...
      courseAudio(uniqueId,recordFile,index){
        ...
        let voiceUrl;
        // let apiUrl = SYS_BUSINESS().downloadVoiceFileReal;
        ...
        this.$axios.get(SYS_BUSINESS().downloadVoiceFileReal,{
          params:{
            uniqueId : uniqueId,
