<template>
<div>
  <o-breadcrumb second="日志监控" third="课程回放"></o-breadcrumb>
              ...
              if(params.row.classRoomUuid){
                return h('div',[
                  h('Button', {
                    ...
                    on: {
                      click: () => {
                        this.videoLog(params.row.courseUuid)
                      }
                    }
                  }),
                  h('Button', {
...
//              <i-button type="success" size="small" icon="eye" title="查看进出日志" @click="log('${row.classRoomUuid}')"></i-button>`;
    ...
    methods: {
      ...
      loadDataGrid(search, orderBy = '', sortBy = '') {
        ...
        //   }
        // });
        this.$axios.get(SYS_BUSINESS().coursePlaybackLog.dataGrid,{
          baseURL: apiHiCRM(),
          params:{
      ...
      videoLog(courseUuid){
        ...
        let _this = this;
        // common.ajax({
        //   url: SYS_BUSINESS().coursePlaybackLog.getTrackDataInfo,
        //   method: 'get',
        //   data:{
        //     token:token,
        ...
        //       _this.$Modal.info({
        //         title:'回放地址',
        //         content:`若浏览器不能自动打开新页面播放视频，<a href= ../../../static/play.html?token=${token}&uuid=${courseUuid} target=_blank>请点击这里</a>观看回放`
        //       });
        //       window.open(`../../../static/play.html?token=${token}&uuid=${courseUuid}`);
        //     }else{
        //       _this.notices(res.code, '获取失败', 'warning');
        //     }
        //   }
        // });
        this.$axios.get( SYS_BUSINESS().coursePlaybackLog.getTrackDataInfo,{
          params:{
            token:token,
            courseUuid:courseUuid,
          }
          ...
          if(data.code == 100){
            ...
            _this.notices(data.code, '获取失败', 'warning');
          }
        })
      },
      getFullPhone(phone) {
