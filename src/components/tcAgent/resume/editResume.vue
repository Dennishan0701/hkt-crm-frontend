...
<div>
        ...
        <div class="base">
          <div class="model">
            <i-form ref="formValidateBase" :model="formItemBase" :rules="ruleValidateBase">
              ...
              <!--</Form-item>-->
              <Form-item prop="agentName" label="代理姓名">
              ...
              <Form-item v-show="!formItemBase.type" prop="parentAgent" label="上级代理">
              ...
              <Form-item prop="agentPhone" label="手机号码">
                <i-input v-model="formItemBase.agentPhone" placeholder="手机号码" style="width:200px"></i-input>
              </Form-item>
              <Form-item class="labelColor" prop="agentEmail" label="电子邮箱">
              ...
              <Form-item class="labelColor" prop="salary" label="薪资">
      ...
      <i-col span="16">
        <div class="detail">
          <i-form ref="formValidateDetail" :model="formItemDetail" :rules="ruleValidateDetail" :label-width="80">
            ...
            <Form-item>
              <i-select v-model="formItemDetail.agentType2" ref="signupSelect" @on-change="changeAgentType" style="width:200px">
              ...
              <p v-show="formItemDetail.agentType2">{{qrcodeOption.url}}</p>
  ...
  <div style="margin-left: 200px;margin-bottom: 10px;">
    <i-button type="primary" icon="android-done" @click="saveResume"> 保 存 编 辑 </i-button>
  </div>
</div>
</template>
...
<script type="text/ecmascript-6">
  ...
  import {formateDate,common,checkImageExt,checkOfficeExt,checkCommonExt} from 'common/js/common.js';
...
//        agentTypeList: [],
    ...
    mounted() {
      ...
      common.ajax({
        url: ZHAO_TEACHER().Agent.agentInfoByEdit,
        method: 'get',
        data: {
          uuid: _this.uuid
        },
        success: function (res) {
          ...
          if (data) {
            ...
            //document.getElementById('headImg').src = data.logoUrl;
            //_this.formItemBase.headImg = data.logoUrl;
            _this.formItemBase.agentName = data.agentName;
            _this.formItemBase.type = (data.generalAgent == "1") ? true : false;
            _this.formItemBase.parentAgent = data.parentAgent;
            _this.formItemBase.agentPhone = data.phone;
            _this.formItemBase.gender = data.sex;
            _this.formItemBase.agentEmail = data.email;
            _this.formItemBase.idCard = data.idNumber;
            _this.formItemBase.salary = data.salary;
            _this.formItemBase.uuid = data.uuid;
            console.log('data.agentArea',data.agentArea);
            if(data.agentArea) {
              _this.formItemDetail.agentSchool = data.agentArea.split(',');
            }
            _this.formItemDetail.schoolLocation = data.schoolLocation;
            if(data.agentType){
              data.agentType.split(',').forEach((item, index) => {
                _this.formItemDetail.agentTypeArray.push(item);
              });
            }
            _this.formItemDetail.bank = data.bankId;
            _this.formItemDetail.bankCard = data.cardNumber;
            _this.formItemDetail.bankAddress = data.bankAddress;

            _this.fileItem.fileIdCardName = data.idNumberUrl;
            _this.fileItem.fileIdCardUrl = data.idNumberUrl;
            _this.fileItem.fileContractName = data.contractUrl;
            _this.fileItem.fileContractUrl = data.contractUrl;
            _this.fileItem.fileOtherName = data.otherUrl;
            _this.fileItem.fileOtherUrl = data.otherUrl;
          } else {
            common.notices('接口返回了空数据,data==null', '提示','error');
          }
        }
      });
      common.ajax({
    ...
    methods: {
        ...
        $.ajax({
          ...
          success: function (res) {
            ...
            if (res.code === '100') {
              ...
              common.notices(res.msg,'上传失败', 'error');
            }
          },
          error: function (res) {
            common.notices(res.statusText, '服务异常', 'error');
          }
        });
      },
      uploadBtn(id) {
      ...
      saveResume() {
        ...
        if (isBaseSuccess && isDetailSuccess) {
          ...
          common.ajax({
            url: ZHAO_TEACHER().Agent.editAgent,
            data: {
              uuid: _this.uuid,
              loginId: _this.loginId,
              agentName: _this.formItemBase.agentName,
              sex: _this.formItemBase.gender,
              parentAgent: _this.formItemBase.type == 1 ? 0 : _this.formItemBase.parentAgent,
      ...
      buildQrOption() {
        let option = {};
        option.url = this.qrcodeUrl + `t/teacher-sign.html?type=${this.formItemDetail.agentType2}&source=${this.formItemBase.uuid}`;
      ...
      changeAgentType(value) {
        this.formItemDetail.agentType2 = value;
