<template>
  <div>
    <o-breadcrumb second="财务" third="已收订金订单"></o-breadcrumb>
      ...
      <Row>
        ...
        <i-col span=24 class="left">
          ...
          <i-select v-model="Search.payType" :size="$store.state.searchModuleSize" clearable placeholder="支付方式"
            ...
            <i-option value="wx_pub_qr">微信扫码</i-option>
            <i-option value="alipay_qr">支付宝扫码</i-option>
  ...
  export default {
    data () {
      ...
      return {
        ...
        tableColumnsChecked: ['leadsName', 'leadsPhone', 'grade', 'courseHour', 'giveLength', 'sumLength', 'money', 'submitter', 'paymentDetailList', 'saleType', 'teamName', 'payType', 'payTime', 'payRemark'],
        total: 0,
        pageIndex: 1,
...
//            return hideCharAtNumber(row.leadsPhone) + ` <Icon type="ios-eye" @click="getFullPhone('${row.leadsPhone}')" class="eye" title="显示全部"></Icon>`;
      ...
      loadDataGrid (search, orderBy = '', sortBy = '') {
        let param = `?studentName=${search.studentName}&orderNo=${search.orderNo}&studentPhone=${search.studentPhone}&payType=${search.payType}&saleName=${search.saleName}&teamName=${search.teamName}&receiptStatus=${search.receiptStatus}&checkStartTime=${search.checkStartTime}&checkEndTime=${search.checkEndTime}&payStartTime=${search.payStartTime}&payEndTime=${search.payEndTime}`;
        ...
        const _this = this;
        this.$axios.get (FINANCE ().Tongji.queryDepositOrderPage + param, {}).then (({data}) => {
      ...
      isSearchNull () {
        ...
        console.log (endTime);
        if (!startTime && !startTime && !studentName && !orderNo && !studentPhone && !payStartTime && !payEndTime && !payType && !saleName && !teamName) {
          return true;
        } else {
          return false;
        }
      ...
      exportData () {
        ...
        apiUrl += `?token=${localStorage.getItem ('token')}&payStartTime=${payStartTime}&payEndTime=${payEndTime}&checkStartTime=${startTime}&checkEndTime=${endTime}`;
