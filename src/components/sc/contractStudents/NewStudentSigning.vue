<template>
  <div class="waitFollowedManager">
    <o-breadcrumb second="销售与客服" third="2~5月新签学生"></o-breadcrumb>
    <div class="grid-widget">
      <Row>
        <i-col span="24" class="right">
          ...
          <i-select :size="$store.state.searchModuleSize" v-model="Search.dateStr" clearable  placeholder="月份" style="width:80px;">
            <i-option value="2018-01-28~2018-02-25">二月</i-option>
            <i-option value="2018-02-26~2018-04-01">三月</i-option>
            <i-option value="2018-04-02~2018-04-29">四月</i-option>
            <i-option value="2018-04-30~2018-05-27">五月</i-option>
          </i-select>
          <!--<Date-picker type="daterange" @on-change="changeDate" ref="daterange" placement="bottom-end" placeholder="签约时间" style="width: 200px"></Date-picker>-->
            ...
            render : (h, params) => {
              if(params.row.stopCourseReminding > 30){
                return h('div',[
                  ...
                  },params.row.stopCourseReminding + '天未跟进')
                ])
//            return `<span class="o-tag red">${row.followDays}天未跟进</span>`;
                ...
                return h('div',[
                  h('span',{},params.row.stopCourseReminding + '天未跟进')
                ])
//            return `<span>${row.followDays}天未跟进</span>`;
              }
            }
    ...
    methods: {
      ...
      searchForm(val) {
        ...
        this.loadDataGrid({
          ...
          startDate: this.Search.dateStr.split('~')[0]||'',
          endDate: this.Search.dateStr.split('~')[1]||''
        });
      },
      loadDataGrid(search, orderBy = '', sortBy = '') {
        let param = `?stuParams=${search.leadsParams}&grade=${search.grade}&signDateStart=${search.startDate}&signDateEnd=${search.endDate}`;
        param += `&pageIndex=${this.pageIndex}&pageNumber=${this.pageNumber}&orderBy=${orderBy}&sortBy=${sortBy}`;
        const _this = this;
        // common.ajax({
        //   url: SCS().MyStudent.queryCrNewSignedStudent + param,
        //   method: 'get',
        ...
        //   }
        // });
        this.$axios.get(SCS().MyStudent.queryCrNewSignedStudent,{
          params:{
            stuParams:search.leadsParams,
            grade:search.grade,
            signDateStart:search.startDate,
            signDateEnd:search.endDate,
