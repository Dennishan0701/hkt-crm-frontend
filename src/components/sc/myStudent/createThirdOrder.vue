  ...
  <div>
    <Modal
      ...
      <i-form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
        ...
        <Form-item label="分期数" prop="coursePriceType">
          <i-select v-model="formValidate.applyTerm" style="width:200px">
            <i-option value="6">6期</i-option>
            <i-option value="12">12期</i-option>
          </i-select>
        </Form-item>
...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    mounted() {
      ...
      // });
      // common.ajax({
      //   url: ORDER().CreateOrder.getLoveHaiMiPayType,
      //   method:'get',
      //   success(res) {
      //     _this.loveHaiMiList = res.data['sysEnumVos'] || [];
      //   }
      // });
    ...
    methods: {
      ...
      onOk(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.$axios.post(ORDER().CreateOrder.createThirdOrder,{
              leadsUuid: this.leadsUuid,
              coursePriceType: this.formValidate.coursePriceType,
              coursePriceActivityType: this.formValidate.coursePriceActivityType,
              buyLength: this.formValidate.buyLength,
              promotionCode: this.formValidate.promotionCode,
              applyTerm: this.formValidate.applyTerm,
