      ...
      <i-form ref='formValidate' :model="formValidate" inline :rules="ruleValidate" :label-width="100">
        <Form-item label="购买课时包" prop="coursePriceType">
          <i-select v-model="formValidate.coursePriceType" @on-change="changeCoursePriceType" style="width:200px"
                    :disabled="isRevise">
            <i-option v-for="(item,index) in coursePriceTypeList" :key='index' :value="item.value">{{ item.name }}
            </i-option>
          </i-select>
        </Form-item>
        <Form-item label="购买课时数" prop="buyLength">
          <Input-number :min="1" v-model="formValidate.buyLength" @on-change='showPay' style="width:200px"
        ...
        <div v-if="isSaved" style="border-bottom: 1px dashed #ccc;">
          <div class="payDetailTitle" style="">付费详情</div>
          <ul>
                ...
                <div class="demo-upload-list" v-for="(items,indexs) in item.fileList" :key='indexs'>
                  <template v-if="items.status === 'finished'">
                    ...
                    <div class="demo-upload-list-cover">
                      ...
                      <Icon type="ios-trash-outline" @click.native="handleRemove2(items,indexs)"></Icon>
                    </div>
                  </template>
                  <template v-else>
        ...
        <div v-show="role == 'cc'">
          ...
          <Form-item label="首课时间" prop="subject">
            <Date-picker type="daterange" v-model='dataTime' size="small" @on-change="changeFirstDate" placeholder="首课时间"
