<template>
  <div>
    <o-breadcrumb second="数据分析" third="报名数据分析"></o-breadcrumb>
...
//        localDistribution:'',//地区分布
    ...
    methods: {
      ...
      loadDataGrid(search, orderBy = '', sortBy = '') {
        let param = `?signUpStartDate=${search.commitStartTime}&signUpEndDate=${search.commitEndTime}`;
        const _this = this;
        // common.ajax({
        //   url: dataAnalysis().analyzeSignUpLeads.dataGrid + param,
        //   method: 'get',
        ...
        //   }
        // });
        this.$axios.get(dataAnalysis().analyzeSignUpLeads.dataGrid,{
          params:{
            signUpStartDate: Search.commitStartTime,
            signUpEndDate: Search.commitEndTime,
          }
        }).then( ({data}) => {
...
//       }
      ...
      gradeDistribution(res){
        ...
        res.forEach(function (v, i) {
          data.push({value: v.count||0, name: v.gradeName})
//          data[i]['value'] = v.count;
//          data[i]['name'] = v.gradeName;
          dataName.push(v.gradeName)
        })
...
//              }
      ...
      top30_city(data){
        ...
        data.forEach(function (v, i) {
          top30_name.push(v.name)
          top30_value.push(v.value)
        })
