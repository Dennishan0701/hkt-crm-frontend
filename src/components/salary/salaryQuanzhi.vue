<template>
  <div>
    <o-breadcrumb second="新增管理" third="等级时薪管理-全职"></o-breadcrumb>
    ...
    <edit ref='edit' @reFresh="reFresh"  nature-of-work="等级时薪管理-全职"></edit>
    <add ref='add' @reFresh="reFresh"  nature-of-work="等级时薪管理-全职"></add>
  </div>
    ...
    methods: {
      ...
      onOk(name,flag) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            let api = flag === 'add' ? SALARY().Level.save : SALARY().Level.update;
            common.ajax({
              url: api,
              data: {
                id: _this.uuid,
      ...
      loadDataGrid(search, pageIndex = 1, orderBy = '', sortBy = '') {
        let param = `?natureOfWork=1&gradeType=${search.gradeType}`;
