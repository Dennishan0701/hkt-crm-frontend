    ...
    >
      <i-form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="80">
        <Form-item label="坐席号" prop="seatsNo">
          <i-select class="seatsNo" :size="$store.state.searchModuleSize" v-model="formItem.seatsNo"
                    placeholder="请选择坐席号" @on-change="changeSeatsNo" style="width:200px">
            <i-option v-for="(item,index) in seatsNoList" :key='item.id' :value="item.voipAccount">{{item.voipAccount}}</i-option>
          </i-select>
          <Button v-clipboard:copy="formItem.seatsNo" v-clipboard:success="onCopy" v-clipboard:error="onError">复制坐席号</Button>
        </Form-item>
        <Form-item label="坐席密码" prop="seatsPassword">
          <i-input :disabled="true" v-model="formItem.seatsPassword" placeholder="" style="width: 200px"></i-input>
        </Form-item>
        <Form-item label="显示号码" prop="hotlinePhone">
          <i-input :disabled="true" v-model="formItem.hotlinePhone" placeholder="" style="width: 200px"></i-input>
        </Form-item>
      </i-form>
    </Modal>
  </div>
    ...
    methods: {
      ...
      ok (name) {
        ...
        this.$refs[name].validate ((valid) => {
          if (valid) {
            _this.$axios.post (COMMON_API ().Phone.binding, {
              token: _this.token,
              userUuid: _this.userUuid,
              voipAccount: _this.formItem.seatsNo,
            }).then (res => {
              if (res) {
      ...
      changeSeatsNo (value) {
        this.formItem.seatsNo = value
        if (this.formItem.seatsNo) {
          this.$axios.post (COMMON_API ().Phone.getUnusePaasAccountInfo, {
            token: this.token,
            voipAccount: this.formItem.seatsNo,
          }).then ((res) => {
      ...
      doCopy () {
        // if(this.formItem.seatsNo){
        //   this.$copyText(this.formItem.seatsNo).then(function (e) {
        //     common.notices('已复制')
        //     console.log(e)
        //   }, function (e) {
        //     common.notices('复制失败')
