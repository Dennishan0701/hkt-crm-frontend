      ...
      <i-form ref='formValidate' :model="formItem" :rules="ruleValidate" :label-width="80">
        ...
        <Form-item label="总监" prop="superLeadsUuid">
          <i-select v-model="formItem.superLeadsUuid" placeholder="部门总监" style="width:200px">
            <i-option v-for="(item,index) in chiefPosts" :key='index' :value="item.userUuid">{{ item.userName }}</i-option>
          </i-select>
        </Form-item>
      ...
      onOk(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            &superLeadsUuid=${_this.formItem.superLeadsUuid}&leaderManageUuid=${_this.formItem.leaderManageUuid}`
            // common.ajax({
